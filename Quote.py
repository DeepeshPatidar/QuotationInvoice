    ...
    def delete_quotation(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                cur = self.db.conn.cursor()
                cur.execute("SELECT pdf_path FROM quotations WHERE id=?", (self.current_quote_id,))
                row = cur.fetchone()
                pdf_path = row[0] if row else None

                # 2. Delete from DB
                cur.execute("DELETE FROM quotation_items WHERE quotation_id=?", (self.current_quote_id,))
                cur.execute("DELETE FROM quotations WHERE id=?", (self.current_quote_id,))
                self.db.conn.commit()
...
if __name__ == "__main__":
    ...
    pdf_maker = QuotePDF(company_info=company, terms=terms, invoice_terms=invoice_terms, logo_path=logo_path, seal_sign_path=seal_sign_path) # This is now generic for both
